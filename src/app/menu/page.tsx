...
// Premium Coffee Shop Menu Items
interface MenuItem {
  id: string;
  name: string;
  price: number;
...
function MenuContent() {
  ...
  const getCustomizedTotalPrice = (basePrice: number) => {
    let price = basePrice;
    if (customSize === 'Grande') price += 0.50;
    if (customSize === 'Venti') price += 1.00;
    if (customMilk === 'Oat Milk (+ $0.50)' || customMilk === 'Almond Milk (+ $0.50)') price += 0.50;

    selectedToppings.forEach(t => {
      if (t.includes('$1.00')) price += 1.00;
      else if (t.includes('$1.50')) price += 1.50;
      else if (t.includes('$0.50')) price += 0.50;
    });
    ...
    <div className="min-h-screen bg-[#0B0A09] text-[#F5F4F0] font-sans overflow-x-hidden pb-32">
      ...
      <main className="max-w-md mx-auto px-4 pt-6 space-y-6">
        ...
        {orderStatus !== 'none' && (
          ...
          >
            ...
            <div className="flex justify-between items-start">
              <div>
                <span className="text-[10px] uppercase font-black text-[#C5A880] tracking-widest">Active Order Live Tracking</span>
                <h3 className="text-lg font-bold text-white mt-1">Sipping is close at hand</h3>
              </div>
              <span className="px-2.5 py-1 bg-[#C5A880]/10 border border-[#C5A880]/20 rounded-full text-[10px] font-bold text-[#C5A880] uppercase tracking-wider">
                ...
                {orderStatus === 'preparing' && 'Crafting'}
            ...
            <div className="mt-6 flex items-center justify-between relative">
              ...
              <div className="flex flex-col items-center z-10">
                ...
                <span className="text-[9px] font-black uppercase mt-1 tracking-wider text-neutral-400">Received</span>
              </div>
              ...
              <div className="flex flex-col items-center z-10">
                ...
                <span className="text-[9px] font-black uppercase mt-1 tracking-wider text-neutral-400">Crafting</span>
              </div>
              ...
              <div className="flex flex-col items-center z-10">
                ...
                <span className="text-[9px] font-black uppercase mt-1 tracking-wider text-neutral-400">Served</span>
              </div>
            </div>
        ...
        <div className="bg-gradient-to-tr from-[#141211] to-[#1C1917] border border-[#C5A880]/15 rounded-2xl p-5 shadow-lg relative overflow-hidden">
          ...
          <div className="flex justify-between items-center">
            <div className="flex items-center gap-2">
              ...
              <span className="text-[11px] font-black uppercase tracking-widest text-neutral-400">Robusta Elite Loyalty</span>
            </div>
          ...
          <div className="mt-3 flex items-baseline justify-between">
            <h4 className="text-2xl font-black text-white">{loyaltyPoints} <span className="text-xs font-semibold text-neutral-400">Points</span></h4>
            <span className="text-xs text-neutral-400">160 pts to next Free Brew</span>
          </div>
        ...
        <div className="relative">
          ...
          <input
            type="text"
            placeholder="Search premium coffee, bakery items..."
            value={searchQuery}
        ...
        <div className="space-y-4">
          <div className="flex items-center justify-between">
            <h3 className="text-xs uppercase font-black tracking-widest text-[#C5A880]">{activeCategory} Selection</h3>
            <span className="text-[11px] text-neutral-500 font-medium">{filteredProducts.length} items available</span>
          </div>
          ...
          {filteredProducts.length === 0 && (
            <div className="text-center py-12 bg-[#141211] rounded-2xl border border-neutral-800">
              <p className="text-neutral-500 text-sm">No items found matching your filter/search.</p>
            </div>
          )}
        </div>
      </main>
          ...
          <div className="max-w-md mx-auto">
            ...
            >
              <div className="flex items-center gap-3">
                ...
                <span className="text-sm font-extrabold uppercase tracking-widest">View Luxury Bag</span>
              </div>
          ...
          <div className="fixed inset-0 z-50 flex items-end justify-center bg-[#0B0A09]/80 backdrop-blur-sm">
            ...
            >
              <div className="sticky top-0 bg-[#12100F] px-5 py-4 border-b border-white/5 flex items-center justify-between z-10">
                <h3 className="text-md font-extrabold uppercase text-white tracking-wider">Customize Order</h3>
                <button
              ...
              <div className="p-5 space-y-6">
                ...
                <div className="flex gap-4 pb-4 border-b border-white/5">
                  ...
                  <div>
                    <h4 className="text-md font-bold text-white">{selectedProduct.name}</h4>
                    <p className="text-xs text-neutral-400 mt-1">{selectedProduct.description}</p>
                    <p className="text-[#C5A880] text-sm font-black mt-1.5">${selectedProduct.price.toFixed(2)}</p>
                  </div>
                </div>
                ...
                {!selectedProduct.category.includes('Pastry') && !selectedProduct.category.includes('Mains') && (
                  <div className="space-y-2.5">
                    <label className="text-xs font-black text-neutral-400 uppercase tracking-widest">Select Size</label>
                    <div className="grid grid-cols-3 gap-2">
                      {(['Standard', 'Grande', 'Venti'] as const).map((sz) => {
                        const surcharge = sz === 'Grande' ? '+$0.50' : sz === 'Venti' ? '+$1.00' : 'Base';
                        return (
                          <button
                            key={sz}
                ...
                {!selectedProduct.category.includes('Pastry') && !selectedProduct.category.includes('Mains') && (
                  <div className="space-y-2.5">
                    <label className="text-xs font-black text-neutral-400 uppercase tracking-widest">Milk Options</label>
                    <div className="grid grid-cols-2 gap-2">
                      {([
                        ...
                        >
                          <span>{mk.split(' (+')[0]}</span>
                          {mk.includes('+$0.50') && <span className="text-[10px] text-[#C5A880]">+$0.50</span>}
                        </button>
                      ))}
                    </div>
                  </div>
                ...
                <div className="space-y-2.5">
                  <label className="text-xs font-black text-neutral-400 uppercase tracking-widest">Premium Add-ons</label>
                  <div className="space-y-2">
                    {[
                      ...
                      return (
                        ...
                        >
                          <span>{top.split(' (')[0]}</span>
                          <span className="text-xs text-[#C5A880] font-black">{top.match(/\+\$.*/)?.[0]?.replace(')', '')}</span>
                        </button>
                      );
                    })}
                  </div>
                ...
                <div className="space-y-2">
                  <label className="text-xs font-black text-neutral-400 uppercase tracking-widest">Special Requests</label>
                  <textarea
                    placeholder="E.g., extra hot, less sweet, light foam..."
                    value={specialNotes}
                ...
                <div className="pt-4 border-t border-white/5 flex items-center justify-between gap-4">
                  ...
                  >
                    ...
                    <span>${getCustomizedTotalPrice(selectedProduct.price).toFixed(2)}</span>
                  </button>
                </div>
              </div>
            </motion.div>
            ...
            >
              <div className="px-5 py-4 border-b border-white/5 flex items-center justify-between">
                <div className="flex items-center gap-2">
                  ...
                  <h3 className="text-md font-extrabold uppercase tracking-wider text-white">Your Selection Bag</h3>
                </div>
                <button
                  ...
                  <div key={item.id} className="bg-neutral-950/60 border border-neutral-800/80 rounded-xl p-4 flex gap-3 relative">
                    ...
                    <div className="flex-1 min-w-0">
                      ...
                      <p className="text-[10px] text-[#C5A880] mt-0.5 font-bold">
                        {item.customizations.size} • {item.customizations.milk.split(' (+')[0]}
                        {item.customizations.toppings.length > 0 && ` • +${item.customizations.toppings.length} Add-on(s)`}
                      </p>
              ...
              <div className="bg-neutral-950 border-t border-white/5 p-5 space-y-4">
                <div className="space-y-1.5 text-xs text-neutral-400">
                  ...
                  <div className="flex justify-between">
                    <span>SaaS Tech Processing Fee</span>
                    <span className="text-white font-semibold">$0.00</span>
                  </div>
                  <div className="flex justify-between border-t border-white/5 pt-2 text-sm">
                    <span className="text-[#C5A880] font-bold">Dine-In Total</span>
                    <span className="text-[#C5A880] font-black">${getCartTotal().toFixed(2)}</span>
                  </div>
                </div>
          ...
          <div className="fixed inset-0 z-50 flex items-center justify-center bg-[#0B0A09]/80 backdrop-blur-sm px-4">
            ...
            >
              ...
              <div className="flex flex-col items-center text-center space-y-4">
                ...
                <div>
                  <h3 className="text-md font-bold text-white uppercase tracking-wider">Waiter Assistance</h3>
                  <p className="text-xs text-neutral-400 mt-1">Need water, clean cutlery, or custom requests? Notify the bar instantly.</p>
                </div>
                ...
                <div className="w-full space-y-2 pt-2">
                  ...
                  >
                    {waiterCalled ? 'Waiter Summoned' : 'Call Waiter Assistance'}
                  </button>
          ...
          <div className="fixed inset-0 z-50 flex items-center justify-center bg-[#0B0A09]/80 backdrop-blur-sm px-4">
            ...
            >
              ...
              <div className="flex flex-col items-center text-center space-y-4">
                ...
                <div>
                  <h3 className="text-md font-bold text-white uppercase tracking-wider">Request Dine-In Bill</h3>
                  <p className="text-xs text-neutral-400 mt-1">Ready to conclude your Robusta Coffee premium experience?</p>
                </div>

                <div className="w-full border border-neutral-800 rounded-xl p-3 text-left space-y-1.5 bg-neutral-950/50">
                  <span className="text-[9px] uppercase font-bold text-neutral-500 tracking-wider">Bill Breakdowns</span>
                  <div className="flex justify-between text-xs">
                    <span>Completed Orders</span>
                    <span className="font-semibold text-white">${getActiveOrderTotal() > 0 ? getActiveOrderTotal().toFixed(2) : '0.00'}</span>
                  </div>
                  <div className="flex justify-between text-xs">
                    <span>Tax (7.5%)</span>
                    <span className="font-semibold text-white">${((getActiveOrderTotal() > 0 ? getActiveOrderTotal() : 0) * 0.075).toFixed(2)}</span>
                  </div>
                  <div className="flex justify-between text-xs pt-1 border-t border-white/5 font-bold text-[#C5A880]">
                    <span>Grand Total</span>
                    <span>${((getActiveOrderTotal() > 0 ? getActiveOrderTotal() : 0) * 1.075).toFixed(2)}</span>
                  </div>
                </div>
          ...
          <div className="fixed inset-0 z-50 flex items-center justify-center bg-[#0B0A09]/80 backdrop-blur-sm px-4">
            ...
            >
              ...
              <div className="flex flex-col items-center text-center space-y-4">
                ...
                <div>
                  <h3 className="text-md font-bold text-white uppercase tracking-wider">Share Experience</h3>
                  <p className="text-xs text-neutral-400 mt-1">Help us fine-tune Vador OS and Robusta Coffee operations.</p>
                </div>
                ...
                <textarea
                  placeholder="Share any thoughts on drink taste, barista service, or menu speed..."
